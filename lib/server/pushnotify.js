...
function init (env, ctx) {
  ...
  function isValidTTL (ttlMinutes) {
    // Numbers only
    if (typeof ttlMinutes !== 'number') return false;
    if (!isFinite(ttlMinutes)) return false;
    
    // Minimum 1 minute, maximum 1 hour
    // (if outside this range, maybe a mixup between minutes and seconds)
    if (ttlMinutes<1) return false;
    if (ttlMinutes>60) return false;
    ...
    if (ctx.pushover) {
      ...
      ctx.pushover.send(notify, function pushoverCallback (err, result) {
        if (err) {
          ...
          var ttlSecs = isValidTTL(notify.dedupTTLminutes)
            ? notify.dedupTTLminutes*60
            : time.mins(15).secs;
          recentlySent.ttl(notify.key, ttlSecs);
  ...
  function notifyToHash (notify) {
    ...
    var dedupFields = notify.dedupFields || ['title', 'message'];
    var info = JSON.stringify(_.pick(notify, dedupFields));
    hash.update(info);
