...
function init (env, ctx) {
  ...
  /**
   * Find the alarm with highest level
  ...
  notifications.findHighestAlarm = function findHighestAlarm (group) {
    ...
    var sorted = _.sortBy(filtered, [function(notification) { -notification.level }]);
  ...
  notifications.requestNotify = function requestNotify (notify) {
    // JIMRANDOMH: If there's a custom sound specified, remove it. (Because this overrides on-phone defaults with horrible results.)
    if ('pushoverSound' in notify)
      delete notify.pushoverSound;
