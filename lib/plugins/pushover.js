...
function init (env, ctx) {
  ...
  pushover.send = function wrapSend (notify, callback) {
    ...
    var retryTime = ctx.levels.isAlarm(notify.level) ? ((notify.level === ctx.levels.URGENT) ? 3 : 20) : 0;
    if ("expiry" in notify)
    ...
    function prepareMessage() {
      var msg = {
        ...
        // JIMRANDOMH: Don't fill in a default sound (Pushover client can provide its own)
