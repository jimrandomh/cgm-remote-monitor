...
function init (ctx) {
  ...
  pump.getPrefs = function getPrefs (sbx) {
    ...
    return {
      ...
      , suspendWarnMins: sbx.extendedSettings.suspendWarnMins || 30
  ...
  pump.checkNotifications = function checkNotifications (sbx) {
    ...
    var data = prepareData(prop, prefs, sbx);
    ...
    if (data.level >= levels.WARN) {
      sbx.notifications.requestNotify({
        ...
        , plugin: pump
        , dedupFields: ['title']
        , dedupTTLminutes: data.level === levels.URGENT ? 30 : 60,
      });
    }

    try {
      var suspendAlert = buildSuspendAlert(prop, data, prefs, sbx);
      if (suspendAlert) {
        sbx.notifications.requestNotify(suspendAlert);
      }
    } catch (error) {
      console.error('Error building suspend alert:', error);
    }
  };
  ...
  function buildSuspendAlert (prop, result, prefs, sbx) {
    console.info('Checking whether to send suspend-alert');
    if (!prefs.suspendWarnMins || prefs.suspendWarnMins <= 0) {
      console.info('Suspend-alert pref is not enabled');
      return null;
    }

    if (!prop || !isSuspendedStatus(prop)) {
      console.info('Not currently suspended');
      return null;
    }

    if (isDataStale(result)) {
      console.info('Stale data');
      return null;
    }

    var suspendInfo = findSuspendInfo(sbx, prefs, prop);
    if (!suspendInfo) {
      console.info('findSuspendInfo returned falsy');
      return null;
    }

    if (suspendInfo.durationMins < prefs.suspendWarnMins) {
      console.info(`Not suspended long enough, suspended for ${suspendInfo.durationMins}/${prefs.suspendWarnMins} mins`);
      return null;
    }

    var roundedDuration = Math.max(1, Math.round(suspendInfo.durationMins));
    var title = 'Pump Suspended ' + roundedDuration + 'm';
    var message = 'Pump has been suspended since ' + timeFormat(suspendInfo.startMoment, sbx) + ' (' + roundedDuration + ' minutes).';

    console.info('Will send suspend alert');
    return {
      level: levels.WARN
      , title: title
      , message: message
      ...
      , group: 'Pump Suspend'
      , plugin: pump
      , notifyhash: 'pump_suspend_warn'
    };
  }

  function isDataStale (result) {
    if (!result || !result.clock || !result.clock.value) {
      return true;
    }
    return result.clock.level > levels.NONE;
  }

  function findSuspendInfo (sbx, prefs, latestStatus) {
    var statuses = collectRecentPumpStatuses(sbx, prefs, latestStatus && latestStatus.device);
    if (_.isEmpty(statuses)) {
      return null;
    }

    var latest = _.last(statuses);
    if (!latest || !isSuspendedStatus(latest)) {
      return null;
    }

    var startMoment = null;
    for (var idx = statuses.length - 1; idx >= 0; idx--) {
      var entry = statuses[idx];
      if (!isSuspendedStatus(entry)) {
        break;
      }
      startMoment = statusMoment(entry);
    }
    ...
    var latestMoment = statusMoment(latest);
    if (!latestMoment) {
      return null;
    }
    ...
    return {
      ...
      , durationMins: moment.duration(latestMoment.diff(startMoment)).asMinutes()
    };
  }

  function collectRecentPumpStatuses (sbx, prefs, deviceUri) {
    var entries = (sbx.data && sbx.data.devicestatus) || [];
    if (!entries.length) {
      return [];
    }

    var urgentClock = prefs.urgentClock || 60;
    var lookbackMinutes = Math.max((prefs.suspendWarnMins || 30) * 4, urgentClock * 2);
    var earliestMills = sbx.time - times.mins(lookbackMinutes).msecs;
    ...
    return _.chain(entries)
      .filter(function(status) {
        ...
        if (deviceUri) {
          if (!status.device || status.device !== deviceUri) {
            return false;
          }
        }
        var entryMills = sbx.entryMills(status);
        if (!entryMills) {
          return false;
        }
        if (entryMills < earliestMills || entryMills > sbx.time) {
          return false;
        }
        return !!statusClockMills(status);
      })
      .sortBy(statusClockMills)
      .value();
  }

  function statusClockMills (status) {
    var pumpClock = _.get(status, 'pump.clock');
    if (pumpClock) {
      var clockMoment = moment(pumpClock);
      if (clockMoment.isValid()) {
        return clockMoment.valueOf();
      }
    }
    return status && status.mills ? status.mills : null;
  }

  function statusMoment (status) {
    var mills = statusClockMills(status);
    return mills ? moment(mills) : null;
  }

  function isSuspendedStatus (status) {
    var pumpStatus = _.get(status, 'pump.status');
    if (!pumpStatus) {
      return false;
    }
    ...
    var statusLabel = pumpStatus.status;
    return typeof statusLabel === 'string' && statusLabel.toLowerCase() === 'suspended';
  }
    ...
    if (pump.status) {
      ...
      if (pump.status.bolusing) {
        ...
        if (pump.warnOnSuspend && pump.status.suspended) {
          result.status = result.status || { label: translate('Status') };
          result.status.level = levels.WARN;
          result.status.message = 'Pump Suspended';
        }
      }
      result.status = _.assign({}, result.status, { value: status, display: status, label: translate('Status') });
    }
  }
